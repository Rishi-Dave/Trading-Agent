...
**Evaluation order at `place`** (ADR-0003 point 1), when more than one gate would refuse the same order: `kill-switch` → `loss-breaker` → `daily-trade-limit` → `whitelist` → `policy-security-type` → `policy-long-only` → `capital-ceiling` → `per-trade-cap` → `preview-required`. Halts first (something has already gone wrong today), then legality (this order should never have been proposed), then sizing (this order is merely too big) — the operator sees the most operationally significant reason first.

### §4.3 State transitions

- **Breaker:** `armed → tripped` when the loss threshold is crossed (event notified, §9). Reset is manual only, requires the operator, and is logged + notified — via either of two equally complete channels (ADR-0003 point 5): a local CLI (`scripts/reset_breaker.py`, typed confirmation + `--operator`) or a phone, entirely through ntfy.sh (`scripts/remote_listener.py`, below) — neither is the "real" path with the other as a fallback; both write identical `caps_state` rows through the same code. Never auto-resets at market open — a new day requires an explicit reset decision. "Today" is the UTC calendar day (ADR-0003 point 2); the once-daily market-open cadence (§9) never straddles the UTC midnight boundary, so this needs no timezone handling.
- **Kill switch:** `engaged ↔ disengaged` via the same two equally complete channels — local CLI (`scripts/kill_switch.py`) or entirely from a phone via ntfy (below); state lives in SQLite (§5.1) so it survives restarts. Engaging also sends a notification. The switch ships **engaged** by default on a fresh database.
- **Remote triggering (ADR-0003 point 5):** breaker reset and kill-switch engage/disengage are fully operable from a phone via an ntfy.sh command topic (`scripts/remote_listener.py`) — every action the local CLIs support (engage, disengage, reset) is triggerable this way; this is not a partial or emergency-only remote surface. Because a push notification cannot perform an interactive typed confirmation, "requires the operator" for this channel is enforced instead by a **TOTP rotating code** (RFC 6238, `etrade_agent/totp.py`, `NTFY_COMMAND_SECRET`) carried in the message body — never a static reusable token, and distinct from the topic name, which is not treated as an authorization credential (ntfy has no per-subscriber confidentiality; a static value sent over the topic would be visible and replayable to anyone who found it, which a code review caught in an earlier revision of this mechanism). Every remote action is logged at the same severity as a local one, triggers its own confirmation notification, calls the identical state-writing code path the local CLIs use (no second enforcement path), and is attributed `changed_by="remote:ntfy"` / `breaker_reset_by="remote:ntfy"` in the audit trail — distinguishing remote from local actions without treating either as more authoritative. Sandbox-only, same as the local CLIs.
- **Loss-breaker P&L source:** unrealized P&L is computed live from `get_positions()` at gate-check time, never from `positions_cache` (ADR-0003 point 3) — the cache is advisory (§5.1) and a stale mark could let the breaker under-trip. `realized_pnl` is populated from executed trades; until the Phase 3 pipeline tracks positions/cost-basis to compute it automatically, it defaults to 0 and the breaker relies on unrealized P&L alone (ADR-0003 point 9).
...
- Secrets live in `.env` only (`ETRADE_CONSUMER_KEY`, `ETRADE_CONSUMER_SECRET`, `ETRADE_SANDBOX`, `NTFY_TOPIC`, `NTFY_COMMAND_SECRET` — a TOTP shared secret, ADR-0003 point 5, required only for `scripts/remote_listener.py`, provisioned via `scripts/generate_totp_secret.py`); access tokens persist to gitignored `tokens/`. Neither ever enters TOML, code, logs, or fixtures (T3).
